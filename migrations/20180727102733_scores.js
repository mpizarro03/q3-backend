...
exports.up = function(knex, Promise) {
  return knex.schema.createTable('scores', table => {
    table.increments()
    table.integer('couple_id').notNullable().defaultsTo()
    table.foreign('couple_id').references('couples.id').onDelete('CASCADE')

    table.integer('from_user_id').notNullable().defaultsTo('')
    table.foreign('from_user_id').references('users.id').onDelete('CASCADE')

    table.integer('to_user_id').notNullable().defaultsTo('')
    table.foreign('to_user_id').references('users.id').onDelete('CASCADE')

    table.integer('users_feelings_id').notNullable().defaultsTo('')
